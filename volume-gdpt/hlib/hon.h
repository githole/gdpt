...
//	typedef IValue* Value;
	...
	struct Response {
		bool valid;
		const T body;
		Response() : valid(false), body() {}
		Response(bool in_valid, T in_body) : valid(in_valid), body(in_body) {}
	...
	struct Response<RealNumber*> {
		...
		Response(bool in_valid, RealNumber* in_body) : valid(in_valid), body(in_body) {}
		...
		const double operator*() {
			if (valid)
				return body->value();
			throw std::runtime_error("Invalid Realnumber");
		}
	};
	...
	struct Response<String*> {
		...
		const String* body;
		Response() : valid(false) {}
		Response(bool in_valid, String* in_body) : valid(in_valid), body(in_body) {}
	...
	public:
		...
		Response<Value> at(const size_t index) {
			if (index >= array_.size()) {
				return Response<Value>(false, NULL);
			}

			return Response<Value>(true, array_[index]);
		}
	};
	...
	struct Response<Array*> {
		...
		const Array* body;
		Response() : valid(false) {}
		Response(bool in_valid, Array* in_body) : valid(in_valid), body(in_body) {}
	...
	public:
		...
		Response<Value> find(const Key& key) const {
			...
			return Response<Value>(true, ite->second);
		}
		...
		Response<RealNumber*> realnumber(const Key& key) const {
			...
			if (response.valid && response.body->type() == ValueTypeRealNumber) {
				return Response<RealNumber*>(true, dynamic_cast<RealNumber*>(response.body.get()));
			}
			return Response<RealNumber*>(false, NULL);
		}
		
		Response<String*> string(const Key& key) const {
			Response<Value> response = find(key);
			if (response.valid && response.body->type() == ValueTypeString) {
				return Response<String*>(true, dynamic_cast<String*>(response.body.get()));
			}
			return Response<String*>(false, NULL);
		}
		
		Response<Map*> map(const Key& key) const {
			Response<Value> response = find(key);
			if (response.valid && response.body->type() == ValueTypeMap) {
				return Response<Map*>(true, dynamic_cast<Map*>(response.body.get()));
			}
			return Response<Map*>(false, NULL);
		}
		
		Response<Array*> valuearray(const Key& key) const {
			Response<Value> response = find(key);
			if (response.valid && response.body->type() == ValueTypeArray) {
				return Response<Array*>(true, dynamic_cast<Array*>(response.body.get()));
			}
			return Response<Array*>(false, NULL);
		}
	...
	private:
		...
		Root parse_internal(const std::vector<TokenInfo>& tokens) {
			...
			if (!cursor.valid() || !cursor.empty())
				return Root(false, Map());

			return Root(true, tmp);
		}
		...
		Response<std::vector<TokenInfo>> tokenize(const std::string& input) {
			...
			return Response<std::vector<TokenInfo>>(valid, ret);
		}
