...
public:
    ...
    virtual double calc_pdf(const Vec &in, const Vec &normal, const Vec &out) const = 0;
...
public:
    ...
    virtual double calc_pdf(const Vec &in, const Vec &normal, const Vec &out) const {
...
public:
    ...
    virtual double calc_pdf(const Vec &in, const Vec &normal, const Vec &out) const {
        ...
        if (cosa < 0)
            cosa = 0.0;
        return (n_ + 1.0) / (2.0 * kPI) * pow(cosa, n_);
    }
...
public:
    ...
    // false: 全反射
    // true: それ以外
    bool calc_dir(const Vec &in, const Vec &normal, Vec *reflect_dir, Vec *refract_dir, double *Fr, double *Ft) const {
        ...
        // 全反射
        *reflect_dir = reflect(in, now_normal);
        if (cos2t_2 < 0.0) {
            return false;
        }
    ...
    virtual Vec sample(Random &random, const Vec &in, const Vec &normal, double *pdf, Color *brdf_value) const {
        ...
        bool internal_reflection = !calc_dir(in, normal, &reflection_dir, &refraction_dir, &Fr, &Ft);
        ...
        if (random.next01() < probability) { // 反射
            ...
            if (brdf_value != NULL) {
                *brdf_value = Ft * eval(in, normal, refraction_dir);
            }
